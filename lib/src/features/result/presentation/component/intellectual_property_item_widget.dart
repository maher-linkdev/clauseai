  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Row(
          children: [
            SeverityBadge(severity: intellectualProperty.severity),
            const SizedBox(width: 8),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
              decoration: BoxDecoration(
                color: _getOwnershipColor(intellectualProperty.ownership).withOpacity(0.1),
                borderRadius: BorderRadius.circular(4),
              ),
              child: Text(
                intellectualProperty.ownership.name.toUpperCase(),
                style: GoogleFonts.lato(
                  fontSize: 10,
                  fontWeight: FontWeight.w600,
                  color: _getOwnershipColor(intellectualProperty.ownership),
                ),
              ),
            ),
          ],
